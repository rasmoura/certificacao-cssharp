...
namespace _01._04
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var item in copiaDaLoja.Filmes)
            {
                Console.WriteLine(item.Titulo);
            }

            var serializerJson = new DataContractJsonSerializer(typeof(LojaDeFilmes));

            using (var file = new FileStream("Loja.json", FileMode.Create, FileAccess.Write))
            {
                serializerJson.WriteObject(file, loja);
            }

            using (var fileStream = new FileStream("Loja.json", FileMode.Open, FileAccess.Read))
            {
                copiaDaLoja = (LojaDeFilmes)serializerJson.ReadObject(fileStream);
            }
